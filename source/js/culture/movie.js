...
$(document).ready(function () {
  if (document.querySelector('.movie-culture-list')) {
    ...
    let init_data = localStorage.getItem('movie_init_data') || '';
    let data_ended = false;
    const cultureList = document.querySelector('.movie-culture-list');
    const loadingElement = document.getElementById('loading');

    if (init_data) {
      localStorage.removeItem('movie_init_data');
    ...
    function restoreScrollPosition() {
      const scrollData = localStorage.getItem('fluid_scroll_position');
      if (scrollData) {
        try {
          const { position, timestamp } = JSON.parse(scrollData);
          if (Date.now() - timestamp < 24 * 60 * 60 * 1000) {
            ...
            setTimeout(() => {
              if (document.documentElement.scrollHeight > position) {
                ...
                localStorage.removeItem('fluid_scroll_position');
              }
            }, 500);
          } else {
            localStorage.removeItem('fluid_scroll_position');
    ...
    const observer = new MutationObserver((mutations) => {
      // 检查是否有新增的电影元素
      const hasNewMovies = mutations.some(mutation =>
    ...
    window.addEventListener('beforeunload', () => {
      ...
      localStorage.setItem('fluid_scroll_position', JSON.stringify(scrollData));
    });
  }
});